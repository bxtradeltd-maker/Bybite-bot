...
def api_grid_backtest():
    """
    Runs the grid/basket EA backtest and returns JSON results - same safe
    pattern as the other backtest routes (calls the function directly,
    no shell/subprocess).

    Example:
      /api/grid_backtest?symbol=BTCUSDT&days=90&direction=buy&repeat=20
    """
    symbol = request.args.get("symbol")
    if not symbol:
        return jsonify({"status": "error", "reason": "missing_symbol_param"}), 400
