...
def compute_signal_series(candles):
    """
    Same state machine as compute_signal(), but runs ONCE across the full
    candle history and records every bar where the base trigger fires
    (not just the last bar). This is what backtesting/sweeping should use
    instead of calling compute_signal() in a sliding window per bar -
    that approach recomputes the entire history from scratch for every
    single bar checked, which is O(n * window) and gets slow fast on
    anything more than a few thousand candles.

    Returns a dict with the fired signals plus the full per-bar arrays
    (atr, rsi, vwap, highs, lows) needed to score each one, without
    needing to touch the network or redo any of this work again.
    """
    n = len(candles)
    if n < (2 * PIVOT_LEN + ST_LEN + 5):
        return {"signals": [], "highs": [], "lows": [], "atr": [], "rsi": [], "vwap": []}
    ...
    signals = []  # list of {"idx": i, "direction": "buy"/"sell"}
        ...
        if i > 0 and active_top is not None and active_buy_ratio is not None:
            ...
            if crossover and not pivot_low_here and \
               active_buy_ratio >= BULL_VOL_PCT and not market_trend_changed:
                signals.append({"idx": i, "direction": "buy"})
        ...
        if i > 0 and active_bot is not None and active_buy_ratio is not None:
            ...
            if crossunder and not pivot_high_here and \
               active_sell_ratio >= BEAR_VOL_PCT and not market_trend_changed:
                signals.append({"idx": i, "direction": "sell"})
...
def htf_trend_series(candles, htf_candles):
    """Maps every bar in `candles` to the HTF trend that was in effect at
    that time (i.e. the trend as of the most recently CLOSED HTF candle).
    Single O(n+m) pass instead of refetching/recomputing per bar."""
    htf_closes = [c["close"] for c in htf_candles]
    htf_ema = _ema_series(htf_closes, HTF_EMA_LEN)
    htf_timestamps = [c["timestamp"] for c in htf_candles]

    trends = [None] * len(htf_candles)
    for i in range(len(htf_candles)):
        if htf_ema[i] is None:
            trends[i] = 0
        else:
            trends[i] = 1 if htf_closes[i] > htf_ema[i] else -1

    result = [0] * len(candles)
    j = -1  # index of the most recent HTF candle closed before/at this bar
    for i, c in enumerate(candles):
        while j + 1 < len(htf_timestamps) and htf_timestamps[j + 1] <= c["timestamp"]:
            j += 1
        result[i] = trends[j] if j >= 0 else 0
    return result


def evaluate_confluence_at(idx, direction, series, htf_trend_at_idx, candles):
    """Same scoring logic as evaluate_confluence(), but pulls its inputs
    from a precomputed signal_series() result instead of recomputing
    diagnostics for a single bar - used by the fast backtest path."""
    ...
    htf_agree = (direction == "buy" and htf_trend_at_idx == 1) or \
                (direction == "sell" and htf_trend_at_idx == -1)

    ote_agree = in_ote_zone(highs, lows, idx, price, direction)
    ...
    breakdown = {
        ...
        "directional": {
            "vwap": vwap_agree, "mkr_trend": mkr_agree, "htf_structure": htf_agree,
            "ote_zone": ote_agree, "fvg": fvg_agree,
        },
    }
